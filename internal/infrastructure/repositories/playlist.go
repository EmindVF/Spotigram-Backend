...
func (spp *SqlPlaylistRepository) GetPlaylists(userId string, offset int) ([]models.Playlist, error) {
	...
	rows, err := db.Query("SELECT id, name, length FROM playlists WHERE user_id = $1 OFFSET $2 LIMIT 100", userId, offset)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&playlists[len(playlists)-1].Name,
			&playlists[len(playlists)-1].Length); err != nil {
			return nil, &customerrors.ErrInternal{Message: err.Error()}
		}
	}
...
func (spp *SqlPlaylistRepository) GetPlaylist(playlistId string) (*models.Playlist, error) {
	...
	db := spp.DBProvider.GetDb()
	row := db.QueryRow("SELECT id, name, length, user_id FROM playlists WHERE id = $1", playlistId)
	if err := row.Scan(&playlist.Id, &playlist.Name, &playlist.Length, &playlist.UserId); err != nil {
		if err == sql.ErrNoRows {
...
func (spp *SqlPlaylistRepository) UpdatePlaylist(p models.Playlist) error {
	...
	stmt, err := db.Prepare("UPDATE playlists SET user_id = $1, name = $2, length = $3 WHERE id = $4")
	if err != nil {
		panic(fmt.Errorf("error preparing UpdatePlaylist SQL statement: %v", err))
	}
	defer stmt.Close()

	_, err = stmt.Exec(p.UserId, p.Name, p.Length, p.Id)
...
func (spp *SqlPlaylistRepository) AddPlaylist(p models.Playlist) error {
	...
	stmt, err := db.Prepare("INSERT INTO playlists (id, user_id, name, length) VALUES ($1, $2, $3, $4)")
	if err != nil {
		panic(fmt.Errorf("error preparing AddPlaylist SQL statement: %v", err))
	}
	defer stmt.Close()

	_, err = stmt.Exec(p.Id, p.UserId, p.Name, p.Length)
	if err == sql.ErrConnDone {
		return &customerrors.ErrInternal{Message: "connection is done"}
	} else if err != nil {
		return &customerrors.ErrInvalidInput{Message: err.Error()}
